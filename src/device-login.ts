export type DeviceLoginToolResult = {
...
function withInstruction(start: DeviceStart, text: string, isError?: true): DeviceLoginToolResult {
  return {
    content: [{ type: "text", text: `${approvalInstruction(start)}\n\n${text}` }],
    isError,
  };
}

export async function runDeviceLoginFlow(options: DeviceLoginOptions): Promise<DeviceLoginToolResult> {
