type ToolResult = {
  content: { type: "text"; text: string }[];
  isError?: boolean;
};

export type DeviceStart = {
  device_code: string;
  user_code: string;
  verification_uri: string;
  verification_uri_complete: string;
  expires_in: number;
  interval: number;
};

type DeviceLoginOptions = {
  siteOrigin: string;
  clientName: string;
  credPath: string;
  persistToken: (accessToken: string, userId?: string) => void;
  fetchImpl?: typeof fetch;
  sleepImpl?: (ms: number) => Promise<void>;
  now?: () => number;
};

type DevicePollOptions = Omit<DeviceLoginOptions, "clientName"> & {
  start: DeviceStart;
};

const defaultSleep = (ms: number) => new Promise((r) => setTimeout(r, ms));

export function approvalInstruction(start: DeviceStart): string {
  return (
    `Open this URL in your browser and approve:\n  ${start.verification_uri_complete}\n` +
    `Your code: ${start.user_code}\n` +
    `(If the link does not prefill, go to ${start.verification_uri} and enter the code.)\n\n` +
    `Waiting for approval...`
  );
}

export async function runDeviceLogin(
  options: DeviceLoginOptions,
): Promise<ToolResult> {
  const fetchFn = options.fetchImpl ?? fetch;
  let start: DeviceStart;
  try {
    const res = await fetchFn(`${options.siteOrigin}/api/mcp/device/start`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Accept: "application/json" },
      body: JSON.stringify({ client_name: options.clientName }),
    });
    if (!res.ok) {
      const text = await res.text();
      return {
        content: [
          {
            type: "text",
            text: `Could not start login (HTTP ${res.status}) from ${options.siteOrigin}/api/mcp/device/start\n\n${text}`,
          },
        ],
        isError: true,
      };
    }
    start = (await res.json()) as DeviceStart;
  } catch (err) {
    return {
      content: [
        {
          type: "text",
          text: `Network error starting login: ${err instanceof Error ? err.message : String(err)}`,
        },
      ],
      isError: true,
    };
  }

  return pollDeviceLogin({ ...options, start });
}

export async function pollDeviceLogin(
  options: DevicePollOptions,
): Promise<ToolResult> {
  const fetchFn = options.fetchImpl ?? fetch;
  const sleepFn = options.sleepImpl ?? defaultSleep;
  const now = options.now ?? Date.now;
  const instruction = approvalInstruction(options.start);
  const deadline = now() + options.start.expires_in * 1000;
  let intervalMs = Math.max(1, options.start.interval) * 1000;

  while (now() < deadline) {
    await sleepFn(intervalMs);
    let res: Response;
    try {
      res = await fetchFn(`${options.siteOrigin}/api/mcp/device/token`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Accept: "application/json" },
        body: JSON.stringify({ device_code: options.start.device_code }),
      });
    } catch {
      continue;
    }

    if (res.ok) {
      const data = (await res.json()) as {
        access_token: string;
        token_type?: string;
        taskbounty_user_id?: string;
      };
      try {
        options.persistToken(data.access_token, data.taskbounty_user_id);
      } catch (err) {
        return {
          content: [
            {
              type: "text",
              text: `${instruction}\n\nLogin succeeded but could not write ${options.credPath}: ${err instanceof Error ? err.message : String(err)}. Set TASKBOUNTY_API_KEY=${data.access_token} in your environment instead.`,
            },
          ],
          isError: true,
        };
      }
      return {
        content: [
          {
            type: "text",
            text:
              `${instruction}\n\nLogged in. Credentials saved to ${options.credPath} (mode 0600).\n` +
              `For CI or headless use, you can also set the env var:\n` +
              `  TASKBOUNTY_API_KEY=${data.access_token}\n\n` +
              `You can now use creator tools like autopilot_enable and post_from_issue.`,
          },
        ],
      };
    }

    let errCode = "";
    try {
      const body = (await res.json()) as { error?: string };
      errCode = body.error ?? "";
    } catch {
      errCode = "";
    }
    if (errCode === "authorization_pending") continue;
    if (errCode === "slow_down") {
      intervalMs += 5000;
      continue;
    }
    if (errCode === "expired_token" || errCode === "access_denied") {
      return {
        content: [
          {
            type: "text",
            text:
              `${instruction}\n\n` +
              (errCode === "access_denied"
                ? "Login was denied in the browser. Run taskbounty_login again to retry."
                : "Login code expired before approval. Run taskbounty_login again to retry."),
          },
        ],
        isError: true,
      };
    }
    return {
      content: [
        {
          type: "text",
          text: `${instruction}\n\nLogin failed (HTTP ${res.status}, error="${errCode}"). Run taskbounty_login again to retry.`,
        },
      ],
      isError: true,
    };
  }

  return {
    content: [
      {
        type: "text",
        text: `${instruction}\n\nLogin timed out waiting for browser approval. Run taskbounty_login again to retry.`,
      },
    ],
    isError: true,
  };
}
