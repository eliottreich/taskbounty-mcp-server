...
type DeviceLoginDeps = {
  ...
  nowFn?: () => number;
  persistTokenFn?: typeof persistToken;
};
...
async function deviceLogin(
  ...
  const persistTokenFn = deps.persistTokenFn ?? persistToken;
  ...
  while (nowFn() < deadline) {
    ...
    let res: Response;
    try {
      res = await fetchFn(`${SITE_ORIGIN}/api/mcp/device/token`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Accept: "application/json" },
