...
export async function runDeviceLogin({
  ...
  fetchFn = fetch,
  sleepFn = sleep,
  persistTokenFn = persistToken,
  siteOrigin = SITE_ORIGIN,
  credPath = CRED_PATH,
  nowFn = Date.now,
}: DeviceLoginOptions): Promise<ToolResult> {
  let start: DeviceStart;
  try {
    const res = await fetchFn(`${siteOrigin}/api/mcp/device/start`, {
      method: "POST",
  ...
  const deadline = nowFn() + start.expires_in * 1000;
  ...
  while (nowFn() < deadline) {
    ...
    let res: Response;
    try {
      res = await fetchFn(`${siteOrigin}/api/mcp/device/token`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Accept: "application/json" },
    ...
    if (res.ok) {
      ...
      try {
        persistTokenFn(data.access_token, data.taskbounty_user_id);
      } catch (err) {
        return {
          content: [
            {
              type: "text",
              text: `${instruction}\n\nLogin succeeded but could not write ${credPath}: ${err instanceof Error ? err.message : String(err)}. Set TASKBOUNTY_API_KEY=${data.access_token} in your environment instead.`,
            },
          ],
          isError: true,
        };
        ...
        content: [
          {
            type: "text",
            text:
              `${instruction}\n\nLogged in. Credentials saved to ${credPath} (mode 0600).\n` +
...
async function deviceLogin(clientName: string): Promise<ToolResult> {
  return runDeviceLogin({ clientName });
}
