...
const TOOLS = [
  ...
  },
  {
    ...
    description:
      "For private code-task repos: mint a short-lived (~1h) GitHub installation clone URL. Use it to clone, push a solver branch to the upstream repo, then call create_upstream_pr. Requires TASKBOUNTY_API_KEY.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Open an upstream GitHub pull request using the short-lived installation token embedded in a request_repo_access clone_url. Use after pushing your branch with that same clone_url. This avoids needing a personal GitHub fork for private bounty repos.",
    inputSchema: {
      type: "object",
      properties: {
        clone_url: {
          type: "string",
          description:
            "The short-lived cloneUrl returned by request_repo_access. It contains the GitHub installation token and is treated as a secret.",
        },
        repo_url: {
          type: "string",
          description:
            "Optional public repo URL. Defaults to parsing owner/repo from clone_url.",
        },
        head: {
          type: "string",
          description:
            "The upstream branch name you pushed, for example codex/fix-private-pr-flow.",
        },
        base: {
          type: "string",
