...
export async function deviceLogin(
  ...
  const persistTokenImpl = options.persistTokenFn ?? persistToken;
  const sleepImpl = options.sleepFn ?? sleep;

  let start: DeviceStart;
  try {
    const res = await fetchImpl(`${SITE_ORIGIN}/api/mcp/device/start`, {
      method: "POST",
  ...
  while (Date.now() < deadline) {
    ...
    let res: Response;
    try {
      res = await fetchImpl(`${SITE_ORIGIN}/api/mcp/device/token`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Accept: "application/json" },
    ...
    if (res.ok) {
      ...
      try {
        persistTokenImpl(data.access_token, data.taskbounty_user_id);
      } catch (err) {
        return {
          content: [
            {
