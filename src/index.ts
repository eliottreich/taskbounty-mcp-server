...
async function deviceLogin(
  ...
  // If no start object provided, start the device flow ourselves (fallback path).
  if (!start) {
    try {
  ...
  // Build the approval instruction if one was not already provided by the caller.
  ...
  switch (name) {
    case "taskbounty_login": {
      ...
      try {
        ...
        // start remains null; deviceLogin() will retry the start POST.
      }

      // Single polling path — deviceLogin handles start/retry and instruction consistently.
